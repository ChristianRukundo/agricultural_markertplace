...
export const orderRouter = createTRPCRouter({
  ...
  create: protectedProcedure.input(createOrderSchema).mutation(async ({ ctx, input }) => {
    try {
      ...
      const ordersByFarmer = validatedItems.reduce(
        (acc, item) => {
      ...
      for (const [farmerId, farmerItems] of Object.entries(ordersByFarmer)) {
        const farmerTotal = farmerItems.reduce((sum, item) => sum + Number(item.priceAtOrder) * item.quantity, 0)
        ...
        if (farmer?.phoneNumber) {
          await smsGateway.sendOrderNotification(farmer.phoneNumber, order.id, "PENDING")
        }
      }
   ...
   */
  getMyOrders: protectedProcedure.input(getOrdersSchema).query(async ({ ctx, input }) => {
    try {
      const userId = ctx.session.user.id
      const userRole = ctx.session.user.role

      const { page, limit, status, paymentStatus, dateFrom, dateTo, sortBy, sortOrder } = input
   ...
   * Get order by ID
   */
  getById: protectedProcedure.input(getOrderByIdSchema).query(async ({ ctx, input }) => {
    try {
      const userId = ctx.session.user.id
      const { id } = input
   ...
   */
  updateStatus: protectedProcedure.input(updateOrderStatusSchema).mutation(async ({ ctx, input }) => {
    try {
      const userId = ctx.session.user.id
      ...
      if (order.seller.phoneNumber) {
        await smsGateway.sendOrderNotification(order.seller.phoneNumber, id, status)
      }
   ...
   * Cancel order
   */
  cancel: protectedProcedure.input(cancelOrderSchema).mutation(async ({ ctx, input }) => {
    try {
      const userId = ctx.session.user.id
      ...
      const notifyUserId = order.sellerId === userId ? order.farmerId : order.sellerId
      const notifyUser = order.sellerId === userId ? order.farmer : order.seller
      ...
      if (notifyUser.phoneNumber) {
        await smsGateway.sendOrderNotification(notifyUser.phoneNumber, id, "CANCELLED")
      }
   ...
   */
  initiatePayment: protectedProcedure.input(getOrderByIdSchema).mutation(async ({ ctx, input }) => {
    try {
      const userId = ctx.session.user.id
      const { id } = input
      ...
      const paymentResult = await paymentGateway.initiatePayment({
        ...
        currency: "RWF",
        customerEmail: order.seller.profile?.contactEmail || order.seller.email,

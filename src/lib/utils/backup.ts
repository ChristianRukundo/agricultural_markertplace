...
export class BackupService {
  ...
  static async createBackup(
    ...
    const { outputDir = this.DEFAULT_BACKUP_DIR, includeUserData = true } =
      options;
    ...
    try {
      ...
      const backupData = {
        ...
        farmerProfiles: includeUserData
          ? await db.farmerProfile.findMany()
          : [],
        sellerProfiles: includeUserData
  ...
  static async getBackupStats(
    backupDir: string = this.DEFAULT_BACKUP_DIR
  ): Promise<{
    totalBackups: number;
    totalSize: number;
    oldestBackup?: Date;
    ...
    try {
      ...
      const backupFiles = files.filter(
        (file) =>
          file.startsWith("agriconnect-backup-") && file.endsWith(".json")
      );
